...
struct GnssMeasurement {
  ...
  GnssMeasurement(const sensor_msgs::msg::NavSatFix::ConstSharedPtr& gnssPosition,
                  const geometry_msgs::msg::TwistStamped::ConstSharedPtr& gnssVelocity)
      : timestamp(std::chrono::seconds(gnssPosition->header.stamp.sec) +
                  std::chrono::nanoseconds(gnssPosition->header.stamp.nanosec)),
        ...
        linearVelocity(Eigen::Vector3d(gnssVelocity->twist.linear.x, gnssVelocity->twist.linear.y,
                                       gnssVelocity->twist.linear.z)) {}
};
